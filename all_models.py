# -*- coding: utf-8 -*-
"""Copy of ALL_Models.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1N1IvYaiEcVoyfIY3TAa1DVa9_KbRBPAe
...
def run_one_epoch(
    ...
    for data_x, data_y in train_loader:
        ...
        print('Training_input Shape',input[0].shape)
    ...
    return metrics_meter,(batch_loss/bs)
...
def test_on_test_data(
    ...
    for data_x, data_y in train_loader:
        ...
        input, label = get_train_inputs(
           input, label, model, batch_size, device, is_rnn)
        #print('TEST INPUT Shape :',input[0].shape)
...
def run_one_epoch_filter(
    ...
    for data_x, data_y in train_loader:
        ...
        data_x = data_x.reshape(-1,seq_len,inp_size).to(device)
...
def init_and_train_model(model,optimiser,lr,momentum,weight_decay, train_loader,test_loader,total_epochs,method='static',lam = 0.001,prox=True,seq_len=5,inp_size=10):
    ...
    if model == 'LEKVAR':
      model = lekvar(input_size=inp_size, seq_len=seq_len).cuda(device=device)
    ...
    for i in range(total_epochs):
      if option == 'LEKVAR':
        ...
        metrics_meter,loss = run_one_epoch_filter(
            ...
            1, is_rnn=False, clip_grad=False, lam = lam,prox=False,inp_size=inp_size,seq_len=seq_len)
        if prox:
...
X_np, beta, GC_1 = simulate_var(p=10, T=3000, lag=2,sparsity=0.7)
X_train_1 = torch.tensor(X_np[np.newaxis], dtype=torch.float32)
X_test_1, beta, GC_test = simulate_var(p=10, T=1000, lag=2, seed=1, GC=GC_1, beta=beta)
X_test_1 = torch.tensor(X_test_1[np.newaxis], dtype=torch.float32)

X_np, beta, GC_2 = simulate_var(p=10, T=3000, lag=2,sparsity=0.2)
X_train_2 = torch.tensor(X_np[np.newaxis], dtype=torch.float32)
X_test_2, beta, GC_test = simulate_var(p=10, T=1000, lag=2, seed=1, GC=GC_2, beta=beta)
X_test_2 = torch.tensor(X_test_2[np.newaxis], dtype=torch.float32)



X_np, beta, GC_3 = simulate_var(p=10, T=3000, lag=2,sparsity=0.4)
X_train_3 = torch.tensor(X_np[np.newaxis], dtype=torch.float32)
X_test_3, beta, GC_test = simulate_var(p=10, T=1000, lag=2, seed=1, GC=GC_3, beta=beta)
X_test_3 = torch.tensor(X_test_3[np.newaxis], dtype=torch.float32)
...
for i in range(0,len(all_X_train)):
  X_train,X_test,Y_train,Y_test = all_X_train[i],all_X_test[i],all_Y_train[i],all_Y_test[i]
  sc = StandardScaler()
  sc = sc.fit(X_train)
...
Losses = []
Train_Losses = []
for datas in range(0,len(train_data_sets)):

  
  models = []
  losses = []
  train_losses = []
  lr_ = [0.001,0.01,0.0001]
  lamd = [1e-7,1e-6,1e-4,1e-5,1e-3,1e-2,0.1]
  ...
  for learning in lr_:
    for lam in lamd:
      model,train_loss,test_loss=init_and_train_model(model='LEKVAR',optimiser='adam',lr=learning,momentum=0.99,weight_decay=None, train_loader=train_data_sets[datas],test_loader=test_data_sets[datas],total_epochs=500,method='static',lam = lam,prox=False,seq_len=5,inp_size=10)
      models.append(model)
...
model_1 = Models[2][np.argmin(Losses[2])]
model_1.GC()
...
for i in model_GC:
  plt.bar(np.arange(1,11),i)
  plt.show()

Estimated_GC = model_GC
def scale(X):
  ...
  return array
Estimated_GC_scaled = scale(Estimated_GC)
Estimated_GC_scaled = (Estimated_GC_scaled >0.5).astype(int)
#Estimated_GC_scaled = (model_GC>0.0075).astype(int)
...
sns.heatmap(Estimated_GC_scaled,ax = ax2,annot=True)
considered = Estimated_GC_scaled
...
all_data = []
overlap = 500;
for ind in range(0,Epilepsy_data.shape[0],1500):
  if ind + 2000 <= Epilepsy_data.shape[0]:
    all_data.append(Epilepsy_data.iloc[ind:ind+2000,:])
  else:
    all_data.append(Epilepsy_data.iloc[ind:,:])

all_data = all_data[:33]
all_data[0]
...
main_dir = '/content/drive/MyDrive/EEGLEKVAR_LAG3_Final'
os.mkdir('/content/drive/MyDrive/EEGLEKVAR_LAG3_Final')
all_models = []
all_losses = []
all_train_losses = []
gc_total_arrays =[]
for datas in range(0,len(all_data)):
  ...
  for learning in lr_:
    for lam in lamd:
      model,train_loss,test_loss=init_and_train_model(model='LEKVAR',optimiser='adam',lr=learning,momentum=0.99,weight_decay=None, train_loader=train_data_sets[datas],test_loader=test_data_sets[datas],total_epochs=1000,method='static',lam = lam,prox=False,seq_len=3,inp_size=19)
      models.append(model)
      losses.append(test_loss)
      train_losses.append(train_loss.detach().cpu().numpy())
...
######## Saving GC
  ...
  gc_total_path = gc_total_path+'.csv'
  df = pd.DataFrame(gc)
...
######## Saving Testing Losses 
  ...
  test_losss = np.array(losses)
  #os.mkdir(test_loss_total_path)
  test_loss_total_path = test_loss_total_path+'.csv'
...
######## Saving Training Losses 
  ...
  train_losss = np.array(train_losses)
  #os.mkdir(train_loss_total_path)

  train_loss_total_path = train_loss_total_path+'.csv'

  train_losss.tofile(train_loss_total_path, sep = ',')

main_dir = '/content/drive/MyDrive/SpectralDataAnalysisCMLPWF_Prox'
#os.mkdir('/content/drive/MyDrive/SpectralDataAnalysisCMLPWF_Prox')
all_models = []
all_losses = []
all_train_losses = []
gc_total_arrays =[]
for datas in range(46,51):
  ...
  for learning in lr_:
    for lam in lamd:
      model,train_loss,test_loss=init_and_train_model(model='cmlpwf',optimiser='adam',lr=learning,momentum=0.99,weight_decay=None, train_loader=train_data_sets[datas],test_loader=test_data_sets[datas],total_epochs=200,method='static',lam = lam,prox=True,seq_len=3,inp_size=19)
      models.append(model)
      losses.append(test_loss)
      train_losses.append(train_loss.detach().cpu().numpy())
